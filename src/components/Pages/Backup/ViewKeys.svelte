<script>
  ...
  import { shortAddress, copyToClipboard } from "../../../common/utils.js";
  ...
  onMount(() => {
    browser.runtime.sendMessage({ type: "decryptKeys", data: {} }).then(
      (accountKeys) => {
      ...
      {#each accounts as account}
        <div class="flex-row key-row">
          ...
          <div class="flex-column result-box text-body3 text-secondary">
            <div>{$_("Wallet address")}: <div><Icon src={mdiContentCopy} class="copy-text" size="1.5" color="grey" on:click={() => copyText(account.address)}/> {account.address}</div></div>
            <div>{$_("Public key")}: <div><Icon src={mdiContentCopy} class="copy-text" size="1.5" color="grey" on:click={() => copyText(account.keyPair.public)}/> {account.keyPair.public}</div></div>
            <div>{$_("Secret key")}: <div><Icon src={mdiContentCopy} class="copy-text" size="1.5" color="grey" on:click={() => copyText(account.keyPair.secret)}/> {account.keyPair.secret}</div></div>
          </div>
        </div>
      {/each}
    {/if}
